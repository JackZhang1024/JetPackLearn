...
//
    ...
    private ConcurrentHashMap<String, StickyLiveData> mHashMap =
     ...
     * 一堆反射 获取LiveData的mVersion字段 来控制数据的分发与否
     * 不够优雅
     */
    ...
    private class WrapperObserver<T> implements Observer<T> {
        ...
        public WrapperObserver(StickyLiveData liveData, Observer<T> observer, boolean sticky) {
            ...
            // 比如先使用StikcyLiveData 发送一条数据
            // StickyLiveData#version = 1
            // 那当我们创建WrapperObserver注册进去的时候，
            // 就至少需要把它的version和StickyLiveData的version
            // 保持一致 用户过滤老数据 否则 岂不是收到老数据了
        ...
        public void onChanged(T t) {
            // 如果当前observer收到数据的次数已经大于等于
            // stickyLiveData发送数据的个数了 则return

            /*
             * observer.mLastVersion >= mLiveData.mVersion
             * 这种情况 只会出现在，我们先行创建一个liveData发射了一条数据。此时liveData的mversion=1.
             *
             * 而后注册一个observer进去。由于我们代理了传递进来的observer,进而包装成wrapperObserver，
             * 此时wrapperObserver的lastVersion 就会跟liveData的mversion 对齐。保持一样。把wrapperObserver注册到liveData中。
             *
