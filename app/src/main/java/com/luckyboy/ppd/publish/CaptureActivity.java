...
import com.luckyboy.jetpacklearn.databinding.ActivityLayoutCaptureBinding;
import com.luckyboy.libcommon.utils.ToastManager;
import com.luckyboy.ppd.core.view.RecordView;
...
public class CaptureActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mBinding.recordView.setListener(new RecordView.onRecordListener() {
            @Override
            public void onClick() {
                takingPicture = true;
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == PreviewActivity.REQ_PREVIEW && resultCode == RESULT_OK) {
            Log.e(TAG, "onActivityResult: 预览返回结果");
    ...
    private void bindCameraX() {
        ...
        if (!hasAvailableCameraId) {
            ToastManager.showToast("无可用的设备 cameraId!,请检查设备的相机是否被占用");
            finish();
            return;
        }
        // 查询一下是否存在可用的cameraId 形如: 后置："0" 前置 "1"
        ...
        // 下面我们要查询一下 当前设备它所支持的分辨率有哪些
        // 然后更新一下 所配置的几个usecase
